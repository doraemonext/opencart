<?php
class ModeltmrevolutoinslideshowBanner extends Model {

	public function upgradeToLatest(){
		/*add custom cod field*/
		$query = $this->db->query("SELECT * FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = '".DB_DATABASE."' AND TABLE_NAME = '".DB_PREFIX."ecbanner_image_description'");
		if($query->num_rows <= 0){
			$query = $this->db->query("ALTER TABLE `".DB_DATABASE."`.`".DB_PREFIX."ecbanner_image_description` ");
		}
		/*add params field*/
	...
	public function addBanner($data) {
		...
		if (isset($data['banner_image'])) {
			foreach ($data['banner_image'] as $banner_image) {
				...
				$this->db->query("INSERT INTO " . DB_PREFIX . "ecbanner_image SET ecbanner_id = '" . (int)$banner_id . "', link = '" .  $this->db->escape($banner_image['link']) . "', image = '" .  $this->db->escape($banner_image['image']) . "', params='".$this->db->escape($params)."', ordering='".$this->db->escape($banner_image['ordering'])."'");

				$banner_image_id = $this->db->getLastId();

				foreach ($banner_image['banner_image_description'] as $language_id => $banner_image_description) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "ecbanner_image_description SET ecbanner_image_id = '" . (int)$banner_image_id . "', language_id = '" . (int)$language_id . "', ecbanner_id = '" . (int)$banner_id . "', title = '" .  $this->db->escape($banner_image_description['title']) . "', description = '" .  $this->db->escape($banner_image_description['description']) . "'");
				}
			}
		}
		return $banner_id;
